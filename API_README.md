...
|-----------|------|----------|---------|-------------|
| `from` | string | ✅ | - | Origin airport code(s), comma-separated for multi-origin (e.g., `PBI` or `PBI,FLL`) |
...
**Example:**
```bash
curl "http://localhost:3000/flights/search/roundTrip?from=PBI,FLL&to=LAS&departDate=2026-02-09"
```
...
**Example:**
```bash
curl "http://localhost:3000/flights/cheapest?from=PBI,FLL&to=LAS"
```
...
**Example:**
```bash
curl "http://localhost:3000/flights/search/oneWay?from=PBI,FLL&to=LAS&departDate=2026-02-09"
```
